...
const BillboardForm: React.FC<BillboardProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: BillboardValues) => {
    try {
      ...
      if (initialData) {
        await axios.patch(
          `/api/stores/${params.storeId}/billboard/${params.billboardId}`,
          data
        );
      } else {
        await axios.post(`/api/stores/${params.storeId}/billboard`, data);
      }
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(
        `/api/stores/${params.storeId}/billboard/${params.billbardId}`
      );
      router.refresh();
      router.push("/");
